...
let make = (~prepareDisposeTimeout=5 * 60 * 1000): array<RelayRouter.Types.route> => {
  ...
  {
    let routeName = "Other"
    let loadRouteRenderer = () => (() => Js.import(Other_route_renderer.renderer))->Obj.magic->doLoadRouteRenderer(~routeName, ~loadedRouteRenderers)
    let makePrepareProps = (. 
    ~environment: RescriptRelay.Environment.t,
    ~pathParams: Js.Dict.t<string>,
    ~queryParams: RelayRouter.Bindings.QueryParams.t,
    ...
    ignore(pathParams)
    ignore(queryParams)
    let prepareProps: Route__Other_route.Internal.prepareProps =   {
      environment: environment,
